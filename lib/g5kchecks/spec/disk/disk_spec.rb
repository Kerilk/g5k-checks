describe "Disk" do
  ...
  RSpec.configuration.node.ohai_description.block_device.select { |key,value| key =~ /[sh]d.*/ and value["model"] != "vmDisk" }.each { |k,v|
    ...
    it "should have the correct vendor" do
      ...
      vendor_api = ""
      vendor_api = @api[k]['vendor'] if (@api and @api[k] and @api[k]['vendor'])
      vendor_api = vendor_api.to_f if vendor_api.to_f != 0.0
      vendor_ohai = vendor_ohai.to_f if vendor_ohai.to_f != 0.0
